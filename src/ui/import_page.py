...
class _ParseWorker(QObject):
    ...
    def run(self) -> None:
        ...
        except Exception as error:  # noqa: BLE001 - worker must report all failures to the UI
...
class ImportPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        filter_row.addWidget(QLabel("国家"))
        filter_row.addWidget(self.country_filter)
        filter_row.addWidget(QLabel("档位"))
        filter_row.addWidget(self.tier_filter)
        filter_row.addWidget(QLabel("问题"))
        filter_row.addWidget(self.severity_filter)
        filter_row.addStretch(1)
        root.addLayout(filter_row)

        splitter = QSplitter()
        self.issue_table = self._table(["级别", "代码", "Sheet/文件", "行", "列", "原值", "说明"])
        self.issue_table.setObjectName("issueTable")
        self.price_table = self._table(
            [
                "国家",
                "USD档位",
                "币种",
    ...
    def _connect_signals(self) -> None:
        ...
        self.country_filter.currentIndexChanged.connect(self._populate_price_table)
        self.tier_filter.currentIndexChanged.connect(self._populate_price_table)
    ...
    def _on_parse_failed(self, message: str) -> None:
        self._clear_preview()
        self.status_label.setText(f"解析发生异常：{message}")
    ...
    def _confirm_current(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, "不能确认入库", str(error))
            return
        ...
        try:
            ...
            self.status_label.setText(f"确认失败，可重试：{error}")
    ...
    def _populate_summary(self) -> None:
        ...
        parts = [
            f"渠道：{self._preview.channel.value}",
    ...
    def _populate_issue_table(self) -> None:
        ...
        for row, issue in enumerate(visible):
            ...
            self._set_table_row(self.issue_table, row, values)

    @Slot()
    def _populate_price_table(self) -> None:
        records = () if self._preview is None else self._preview.records
        country = self.country_filter.currentData() or "ALL"
        tier = self.tier_filter.currentData() or "ALL"
        visible = [
            record
            for record in records
            if (country == "ALL" or record.country_code == country)
            and (tier == "ALL" or record.usd_tier == Decimal(str(tier)))
        ]
        self.price_table.setRowCount(len(visible))
        for row, record in enumerate(visible):
            values = (
                ...
                record.adjustment_mode.value if record.adjustment_mode is not None else "",
                record.source_sheet,
                str(record.source_row),
                record.source_column,
            )
            self._set_table_row(self.price_table, row, values)
